...
每次应用启动完成 catalog hydrate 后，`activeWorktreeId` 保持 `null`。只有用户点击某个 worktree，或通过 Open Project 新增项目后，才进入具体 checkout。
...
- 后续 catalog refresh 若当前已有有效选择，应保留当前选择；只有首次启动 hydrate 强制为空。
