...
- `electron/domains/chat/spireSettings.ts` 及测试：只读取 `~/.spirecode/settings.json`，使用内存 SettingsManager 隔离 Pi 默认 settings。
- `electron/domains/chat/bundledResources.ts` 及测试：解析并校验 bundled resources，合并 SpireCode 用户 sources，阻止同 package 与实际注册名冲突。
...
3. 实现 `~/.spirecode/settings.json` loader；移除 packages/extensions 后构造内存 SettingsManager，避免读取 Pi 默认和项目 settings。
4. 实现 bundled resolver：校验 bundle 完整性，合并 SpireCode 用户 sources，并按 package identity 去除 bundled 重复项。
5. 使用 SDK `noExtensions: true` 关闭全局/项目自动发现，通过 `additionalExtensionPaths` 显式加载 bundled 与 SpireCode 用户资源。
6. 在 SDK 单次执行 extension factories 后，以 `extensionsOverride` 检查实际 tool/command/provider 冲突；出错时不创建 AgentSession。
7. 将 staging tree 作为 `extraResources` 放在 ASAR 外，接入开发、打包、签名和 smoke 流程。
8. 运行针对性测试、真实 SDK 探针、`pnpm check` 和 `pnpm bundle`。

## Risks

- 配置隔离不完整是首要风险；真实 SDK 测试覆盖 Pi 全局 settings、全局 extensions 和项目 extensions 三条入口。
