...
- `electron/domains/worktrees/index.ts`：删除 managed worktree 后删除 local branch，调整 create rollback 避免重复删除。
- `electron/domains/worktrees/worktrees.test.ts`：local branch 删除和 rollback 集成断言。
...
5. 更新删除对话框断言与文案，明确 local branch 会被一并删除。
6. 修改 Main 的删除事务：worktree remove 成功后以 Git 参数数组强制删除准确 branch，再删除 catalog record；`rollbackCreated` 不再重复删 branch。
7. 更新真实临时 Git fixture，证明普通删除和 force 删除都移除 local branch，create rollback 仍无 checkout/branch/catalog 残留。
...
- 删除 branch 是不可逆行为，尤其未合并 branch；确认文案必须明确，后端仅对 catalog 中的 managed branch 执行，且绝不触碰 remote branch。
- `git worktree remove` 成功后 branch/catalog 操作失败会形成部分完成状态，这是现有 Git 多步操作的固有风险；返回结构化 recovery 信息，不伪装成功。替代方案“先删 branch”不可行，因为 branch 仍被 worktree checkout；“保留 branch”则不满足需求。
...
验收结果：所有项目初始仅显示 Project 行；点击后显示其 worktrees；Base branch 可搜索并只提交合法 origin ref；删除 managed worktree 后 `refs/heads/<branch>` 不存在。
