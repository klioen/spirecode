...
- `package.json`：增加平台 build target、图标配置和可独立调用的平台 bundle/smoke scripts。
- `assets/icon.ico`、`assets/icons/*.png`：Windows 和 Linux 应用图标。
- `scripts/package-electron.mjs`：从 macOS 固定流水线改为目标宿主平台分派，同时保持公共准备步骤。
- `scripts/smoke-windows.ps1`：验证 Windows unpacked app、asar、Pi 扩展、node-pty、Pi SDK 和 GUI 启动。
- `scripts/smoke-linux.sh`：验证 Linux unpacked app、asar、Pi 扩展、node-pty、Pi SDK 和 GUI 启动。
...
4. 增加 Windows/Linux artifact smoke；复用现有 Pi 扩展校验器和 asar CLI，实测最终 artifact 内的原生模块与 SDK。
...
- Windows 路径、PowerShell quoting 和 executable 名称与 Unix 不同；Windows smoke 必须完全使用参数数组和 PowerShell 原生路径 API。
