...
## Proposed outcome
在 Settings 中增加 Memory 模块，只读展示 `memory_summary.md` 和 `MEMORY.md`，并允许配置 Memory 提取模型及 Phase 1 Reasoning Effort。用户可在两个文档之间切换，无需打开项目或访问文件系统。
...
- 配置范围只包含 Memory Model 和 Phase 1 Reasoning Effort，不增加 Recall、Automatic Processing、token limit 或 Phase 2 reasoning 等其他设置。
